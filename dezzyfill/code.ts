...
async function setText(node: TextNode, val: string, start?: number, end?: number): Promise<void> {
  // Load fonts before editing — deduplicate to avoid loading the same font per character
  if (node.fontName === figma.mixed) {
    const len = node.characters.length;
    const seenFonts: Set<string> = new Set();
    for (let i = 0; i < len; i++) {
      const rangeFont = node.getRangeFontName(i, i + 1);
      if (rangeFont !== figma.mixed) {
        const key = rangeFont.family + "\x00" + rangeFont.style;
...
async function autoApplyTaggedNodes(nodes: readonly SceneNode[], tag: string, data: Record<string, string>): Promise<void> {
  if (!tag) return;

  const isImageTag = tag.startsWith("img:");
  const imageIdx = isImageTag
    ? (tag === "img:logo" ? "logo" : tag.match(/^img:(\d+)$/) ? tag.match(/^img:(\d+)$/)![1] : tag.substring(4))
    : null;

  // Build the data map the same way applyDataAll does so statsCombo / featuresBulleted work
  const map: Record<string, string> = {};
  for (const k in data) {
    if (Object.prototype.hasOwnProperty.call(data, k)) map[k] = data[k];
  }
  map.price = formatPrice(data.price || "");
  const beds = (data.bedrooms || "").trim();
  const baths = (data.bathrooms || "").trim();
  const sqft = (data.sqft || "").trim();
  const parts: string[] = [];
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    if (isImageTag && imageIdx !== null && "fills" in n) {
      const hash = images[imageIdx];
      if (!hash) continue;
      try {
        const image = figma.getImageByHash(hash);
        if (!image) continue;
        const fills = (n as any).fills as readonly Paint[];
        const nonImageFills: Paint[] = [];
        let existingFilters: any = undefined;
        for (let j = 0; j < fills.length; j++) {
          if (fills[j].type === "IMAGE") {
            if ((fills[j] as ImagePaint).filters !== undefined) existingFilters = (fills[j] as ImagePaint).filters;
          } else {
            nonImageFills.push(fills[j]);
          }
        }
        (n as any).fills = nonImageFills;
        const newFill: any = { type: "IMAGE", imageHash: hash, scaleMode: "FILL" };
        if (existingFilters !== undefined) newFill.filters = existingFilters;
        nonImageFills.unshift(newFill);
        (n as any).fills = nonImageFills;
      } catch (e) {
        console.log("Auto-apply image error: " + String(e));
      }
    } else if (!isImageTag && n.type === "TEXT") {
      if (Object.prototype.hasOwnProperty.call(map, tag)) {
        await setText(n as TextNode, map[tag]);
      }
    }
  }
}
...
async function detectExistingImages(): Promise<void> {
  if (detectingImages) return;
  detectingImages = true;

  // Collect outside try so finally can always send the completion message
  const detectedImages: Array<{ position: number | string; hash: string }> = [];
  ...
  try {
    ...
    for (let i = 0; i < nodes.length; i++) {
      const n = nodes[i];
      if (!n || !("fills" in n) || n.locked) continue;
      ...
      for (let j = 0; j < fills.length; j++) {
        ...
        if (fill && fill.type === "IMAGE" && fill.imageHash) {
          ...
          if (!images[key]) {
            images[key] = fill.imageHash;
            detectedImages.push({ position: position as number | string, hash: fill.imageHash });
          }
          break;
        }
      }
    ...
    // Sort numeric positions for a clean display order
    detectedImages.sort((a, b) => {
      if (a.position === "logo") return 1;
      if (b.position === "logo") return -1;
    ...
    // Always send — even on error — so the UI never hangs waiting
    figma.ui.postMessage({ type: "EXISTING_IMAGES_DETECTED", images: detectedImages });
  }

  // Background: fetch actual bytes for each detected hash so UI can show thumbnails.
  // Runs AFTER the UI is unblocked — each image gets a 5s timeout to avoid hangs.
  if (detectedImages.length > 0) {
    fetchImagePreviews(detectedImages);
  }
}

async function fetchImagePreviews(detected: Array<{ position: number | string; hash: string }>): Promise<void> {
  for (let i = 0; i < detected.length; i++) {
    const item = detected[i];
    try {
      const img = figma.getImageByHash(item.hash);
      if (!img) continue;
      const bytes = await Promise.race<Uint8Array | null>([
        img.getBytesAsync(),
        new Promise<null>((resolve) => setTimeout(() => resolve(null), 5000))
      ]);
      if (bytes) {
        figma.ui.postMessage({ type: "EXISTING_IMAGE_PREVIEW", position: item.position, bytes });
      }
    } catch (e) {
      // Skip — timeout or missing image; slot stays as "on canvas" indicator
    }
  }
}
...
async function detectExistingData(): Promise<void> {
  ...
  try {
    ...
    detectingData = false;
    // Always send — even on error — so the UI never hangs waiting
    figma.ui.postMessage({ type: "EXISTING_DATA_DETECTED", data: dataMap });
  }
}
...
figma.ui.onmessage = async (msg: any) => {
  ...
  // Clear all non-logo images (sent before IMAGE_ADD batch)
  if (msg.type === "IMAGES_CLEAR") {
    const logoHash = images["logo"];
    images = {};
    if (logoHash) images["logo"] = logoHash;
    return;
  }

  // Add a single image by position — bytes may arrive as Uint8Array (structured clone) or number[]
  if (msg.type === "IMAGE_ADD") {
    const position = msg.position;
    const raw = msg.bytes;
    if (position && raw && raw.length > 0) {
      try {
        ...
        figma.ui.postMessage({ type: "IMG_STATUS", text: "Failed to load image " + position + ": " + String(e) });
      }
    }
    return;
  }

  // Preserve a canvas-detected hash without re-uploading bytes
  if (msg.type === "IMAGE_KEEP") {
    const position = msg.position;
    const hash = msg.hash;
    if (position && hash) {
      images[String(position)] = hash;
    }
    return;
  }

  // Sent by UI after all IMAGE_ADD/IMAGE_KEEP messages — confirm total loaded
  if (msg.type === "IMAGES_BATCH_DONE") {
    const total = Object.keys(images).filter(k => k !== "logo").length;
    figma.ui.postMessage({ type: "IMG_STATUS", text: "Ready — " + total + " image" + (total === 1 ? "" : "s") + " loaded." });
    return;
  }
  ...
  if (msg.type === "SET_TAG_FROM_UI") {
    ...
    // Auto-apply the matching content immediately after tagging
    autoApplyTaggedNodes(sel, tag, msg.data || {}).catch(function(e) {
      figma.ui.postMessage({ type: "STATUS", text: "Auto-apply error: " + String(e) });
    });
