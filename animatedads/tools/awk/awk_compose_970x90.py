...
# ---- geometry
STRIP=220; PAN=30; COPY_X=236; COPY_W=256; MID=45
LW,LH=map(float,open(f"{SC}/awk_logo_stacked.dims").read().split())   # stacked lockup
LOGO_W=160; logo_s=LOGO_W/LW; LOGO_H=LH*logo_s; logo_x=970-12-LOGO_W; logo_y=MID-LOGO_H/2
...
def fit(lines):                      # largest size whose widest line spans exactly COPY_W
    return min([COPY_W/sum(width(D,w,1) for D,w in ln) for ln in lines]+[26.0])   # capped for the 90 px stage
...
Q=min(20.0, fit([[(W,"Which builders are paying")],[(WBI,"closing costs"),(W," this month?")]])); Q_LH=1.15*Q   # end frame: three rows must fit 90 px
...
title=os.environ.get("AWK_TITLE","AgentWhoKnows 970x90")
html=f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<style>
  ...
  #ad {{ position:relative; width:970px; height:90px; overflow:hidden; background:{NAVY}; }}
...
<div id="ad" class="no-autoplay">
  <div id="photo">
    <img id="bgstill" src="bg-970x90-still.jpg" width="{STRIP+PAN}" height="90" alt="">
    <video src="bg-970x90.mp4" poster="bg-970x90-still.jpg" autoplay muted loop playsinline preload="auto" webkit-playsinline disableremoteplayback></video>
  </div>
  <svg viewBox="0 0 970 90" fill="none" xmlns="http://www.w3.org/2000/svg">
...
print(f"970 index.html {len(html)} bytes | copy {COPY_X}-{COPY_X+COPY_W} | CTA {cta_x:.0f}-{cta_x+CTA_W:.0f} | logo {logo_x:.0f}-{logo_x+LOGO_W:.0f} | f1 baselines {b1:.1f}/{b2:.1f} f2 {r1:.1f}/{r2:.1f} f3 {q1:.1f}/{q2:.1f}/{rb:.1f}")
