...
ADS=[(f"APERTURE_PrioryWalk_{s}_{kind}", s, kind) for kind in ("video","carousel") for s in ORDER if os.path.isdir(f"APERTURE_PrioryWalk_{s}_{kind}")]
def embed(folder):
...
def card(f,size,kind):
    ...
    <section class="card" style="--w:{max(int(w),300)}px">
      ...
      <div class="meta">
        ...
        <p class="notes">{NOTES.get(size,"") if kind=="carousel" else "Video version: clips 1-6 in order, 2.5 s each as a 15 s silent H.264 loop; if autoplay is refused the 4-photo carousel underneath takes over as the fallback."}</p>
...
page=f"""<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><title>Aperture · Priory Walk · Kensington · carousel builds</title>
<style>
...
<header><h1>Aperture <span>· Priory Walk · Kensington · carousel &amp; video · {len(ADS)} units</span></h1>
...
if not SITE:
    ...
    open(f"{S}/README.txt","w").write("""Aperture Global Real Estate - Priory Walk - HTML5 carousel ads, review site
